...
import shutil
import inspect
from model.stst import STST_option
...
def model_choose(args, block):
    m = args.model
    if m == 'stst':
        model = STST_option(num_class=args.class_num, **args.model_param)
        shutil.copy2(inspect.getfile(STST_option), args.model_saved_name)
    else:
    ...
    if args.last_model is not None:
        ...
        if type(args.device_id) is list and  len(args.device_id)>0:
            ...
            pretrained_dict = torch.load(args.last_model, map_location='cpu')
